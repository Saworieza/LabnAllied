class OrderConfirmationPdf < Prawn::Document
  def initialize(order_confirmations)
    super()
...
=end
  ...
  def table_content
    ...
    table order_confirmations_rows do
  ...
  def order_confirmations_rows
    [['Order id', 'Name', 'Price']] +
      @order_confirmations.map do |order|
      [order.id, order.sellers, order.sellers_bankers, order.buyers_bankers]
    end
  end
end